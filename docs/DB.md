...
Prisma 7 では全接続でドライバーアダプターが必須です。`PrismaLibSql` が直接接続設定を受け取り、内部でlibSQLクライアントを生成します。
環境変数 `TURSO_DATABASE_URL` と `TURSO_AUTH_TOKEN` が設定されている場合はTursoに接続し、それ以外はローカルSQLiteを使用します。

> **Note**: `prisma.config.ts`（プロジェクトルート）にもdatasource URLを定義しています。これはPrisma CLIコマンド（migrate等）用であり、ランタイムの接続はアダプター経由です。
